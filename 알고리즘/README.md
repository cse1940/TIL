...
- 너비우선탐색(그래프 + 큐)
  ...
  : 그래프의 너비를 우선으로 탐색하기 위해 ```큐```의 개념을 활용한다.

2. 깊이우선탐색(DFS)

- 시작 정점으로부터 갈 수 있는 하위 정점까지 가장 깊게 탐색하고, 더 이상 갈 곳이 없다면 마지막 갈림길로 돌아와서 다른 정점을 탐색하며 결국 모든 정점을 방문하는 순회 방법 

  - 미로 탈출로를 생각하면 이해하기 쉬움

    : ```어느 한 쪽 길로 가장 깊게 들어갔다가 막히면 다시 돌아와서 다른 길을 탐색```함

- 특징
  1. 모든 정점을 방문할 때 유리함. 경우의 수, 순열과 조합 문제에서 많이 사용한다.
  2. 너비우선탐색(BFS)에 비해 코드 구현이 간단하다.
  3. 단, 모든 정점을 방문할 필요가 없거나 최단 거리를 구하는 경우에는 너비우선탐색(BFS)이 유리하다.

3. DFS의 동작 과정

- 그래프는 인접 행렬 or 인접 리스트 방식으로 표현할 수 있다.
- 각 정점을 방문했는지 여부를 판별할 방문 체크 리스트가 필요하다.
  - 사람과 달리 컴퓨터는 각 정점에 방문했는지 여부를 알 수 없으므로 ```visited``` 리스트를 따로 선언하여 각 정점을 방문했는지 체크한다.
...
4. DFS의 구현 방식
   ...
   : DFS를 코드로 구현해보자! 여기서는 인접 리스트로 표현한 그래프를 기준으로 설명한다.
